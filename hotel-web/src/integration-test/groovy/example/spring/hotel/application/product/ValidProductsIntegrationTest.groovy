...
class ValidProductsIntegrationTest extends Specification{
    ...
    def "판매 종료날짜가 지난 상품은 보여주지 않는다."()   {
        productName = "ShowOnlyValidProductsSpec.판매종료상품"
        given: "판매 종료 날짜가 이틀 지난 상품이다."
        productManager.createNewProduct(new Product(
    ...
    def "판매 종료날짜가 없는 경우는 무기한 판매 상품이기에 보여준다"()   {
        productName = "ShowOnlyValidProductsSpec.무기한판매상품"
        ...
        productManager.createNewProduct(
                Product.builder()
                        ...
                        .validProduct(true)
                        .outOfStock(false)
                        .startSaleDateTime(LocalDateTime.now().minusDays(1))
                        .build())
    ...
    def "판매 시작날짜가 없는 경우는 즉시 판매 상품이기에 보여준다"()   {
        productName = "ShowOnlyValidProductsSpec.판매날짜없는상품"

        given: "판매날짜가 없는 상품 등록"
        productManager.createNewProduct(
                new Product(productName: productName, endSaleDateTime: LocalDateTime.now().plusDays(10), validProduct: true, outOfStock: false))
    ...
    def "invalid로 마크된 상품은 보여주지 않는다."()  {
        productName = "ShowOnlyValidProductsSpec.invalid상품"

        given: "invalid한 상품으로 변경"
        Product product = productManager.createNewProduct(new Product(productName: productName, validProduct: false))
    ...
    def "오늘날짜가 판매날짜 기간이 아니면 상품을 보여주지 않는다."()    {
        productName = "ShowOnlyValidProductsSpec.판매기간이아닌상품"
    ...
    def "품절된 상품은 보여서는 안된다"()    {
        productName = "ShowOnlyValidProductsSpec.품절상품"

        given: "상품이 품절되었다"
        Product product = productManager.createNewProduct(
                new Product(productName: productName, outOfStock: true))
         productManager.invalidateProduct(product.getProductId())
