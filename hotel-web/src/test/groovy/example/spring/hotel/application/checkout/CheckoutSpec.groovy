...
class CheckoutSpec extends Specification {
    ...
    private PurchaseHistoryService purchaseHistoryService = Stub(PurchaseHistoryService)
    ...
    void initCheckoutService()  {
        checkoutService = new CheckoutService(purchaseHistoryService)
    }

    def "즉시 구매에서 checkout 시에 구매 상품은 예약 날짜가 지정되어 있어야 한다"() {
        given: "예약날짜 없이 상품 선택"
        Product product = new Product(productId: 1L, se)
        CheckoutItem checkoutItem = new CheckoutItem(new Product(), bookingDateTime: null)
        CheckoutService checkoutService =
                new CheckoutService(purchaseHistoryService)
    ...
    def "장바구니에서 checkout 시에 해당 상품의 예약날짜에 예약이 가능해야 한다"()    {
        ...
        CheckoutItem checkoutItem = new CheckoutItem(new Product(productId: 1L), bookingDateTime: LocalDateTime.now())
    ...
    def "checkout 시에 모든 상품이 판매 가능하여야 한다[성공케이스]"()   {
        ...
        CheckoutItem checkoutItem1 = new CheckoutItem(new Product(productId: 1L), bookingDateTime: LocalDateTime.now())
        CheckoutItem checkoutItem2 = new CheckoutItem(new Product(productId: 1L), bookingDateTime: LocalDateTime.now())
        and: "상품이 모두 판매 가능하다"
    ...
    private List<CheckoutItem> createCheckoutItemsWithOptions() {
        ...
        CheckoutItem item1 = builder.checkoutItem(productId: new Product(productId: 1L), productPrice:1000L, bookingDateTime: LocalDateTime.now()) {
            checkoutProductOption(optionPrice: 100)
        }
        // total price for item1 -> 2200원
        CheckoutItem item2 = builder.checkoutItem(productId: new Product(productId: 1L), productPrice:2000L, bookingDateTime: LocalDateTime.now()) {
            checkoutProductOption(optionPrice: 200)
        }
        // total price for item1 -> 3400원
        CheckoutItem item3 = builder.checkoutItem(productId: new Product(productId: 1L), productPrice:3000L, bookingDateTime: LocalDateTime.now()) {
            checkoutProductOption(optionPrice: 200)
            checkoutProductOption(optionPrice: 200)
        }
